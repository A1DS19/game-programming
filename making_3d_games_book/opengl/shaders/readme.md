⏺ Vertex Shader: Processes each vertex individually.
  ...
  Fragment Shader: Processes each pixel (fragment) on the screen.
  ...
  Example: A triangle with 3 vertices = 3 vertex shader calls, but might generate
  thousands of fragment shader calls (one per pixel the triangle covers).

> so I pass vertices to vertex shader -> in gpu rasterizer converterts the vertices 
  to pixels -> fragment shader asigns color to pixel

⏺ Exactly! That's the core 3D graphics pipeline:
  ...
  The rasterizer is the bridge between geometry (vertices/triangles) and pixels (what
   you see on screen).
